...
interface File { id: string; title: string; description: string; content: string; subject: string; fileUrl: string | null; isApproved?: boolean; author: { id: string; username: string; isAdmin: boolean }; createdAt: string; }
...
export default function MateriaalPage() {
  ...
  // Add modal (available for all users)
  ...
  return (
    <div className="page-container">
      ...
      <main style={{ position: 'relative', zIndex: 1, maxWidth: '1200px', margin: '0 auto', padding: '2rem' }}>
        ...
        {user.isAdmin && files.some(f => !f.isApproved && !f.author.isAdmin) && (
          <div style={{ padding: '1rem', background: 'rgba(251,191,36,0.1)', borderRadius: '12px', marginBottom: '1rem', color: '#fbbf24' }}>
            ⏳ Er is materiaal dat wacht op goedkeuring. Ga naar <Link href="/admin" style={{ textDecoration: 'underline' }}>Admin</Link> om te beoordelen.
          </div>
        )}
        ...
        {filteredFiles.length === 0 ? (
          <div className="card" style={{ padding: '3rem', textAlign: 'center' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.5)' }}>
              Voeg lesmateriaal toe via de knop rechtsboven.
            </p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
            {filteredFiles.map(file => {
              const isPending = !file.isApproved && !file.author.isAdmin;
              return (
                <div 
                  key={file.id} 
                  className="card" 
                  style={{ 
                    ...
                    borderLeft: isPending ? '4px solid #fbbf24' : 'none'
                  }}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      {isPending && <span style={{ color: '#fbbf24', fontSize: '0.75rem' }}>⏳ Pending</span>}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)' }}>
                    <span>Door {file.author.username}{!file.author.isAdmin && !file.isApproved && ' ⏳'}</span>
          ...
          <div className="modal-content" style={{ maxWidth: '700px', maxHeight: '80vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
              <div>
                ...
                {selectedFile.isApproved === false && !selectedFile.author.isAdmin && (
                  <span style={{ background: 'rgba(251, 191, 36, 0.2)', color: '#fbbf24', padding: '0.25rem 0.75rem', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 500, marginLeft: '0.5rem' }}>⏳ Wacht op goedkeuring</span>
                )}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              {(user.isAdmin || selectedFile.author.id === user.id) && (
          ...
          <div className="modal-content" style={{ maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            {!user?.isAdmin && (
              <p style={{ color: '#fbbf24', fontSize: '0.875rem', marginBottom: '1rem', padding: '0.75rem', background: 'rgba(251,191,36,0.1)', borderRadius: '8px' }}>
                ⏳ Jouw materiaal moet eerst worden goedgekeurd door de admin voordat het zichtbaar is voor iedereen.
              </p>
            )}
