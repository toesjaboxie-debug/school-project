  ...
  const handleSaveEmail = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (user) setUser({ ...user, email, emailVerified: false });
      } else {
        setError(data.error || 'Er is een fout opgetreden');
      }
    } catch (e: any) {
  ...
  const handleSendVerification = async () => {
    ...
    try {
      const res = await fetch('/api/auth/verify-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: user.id, email, username: user.username }),
      });
      const data = await res.json();

      if (res.ok) {
        setMessage('Verificatie e-mail verstuurd! Check je inbox.');
      } else {
        setError(data.error || 'Kon e-mail niet versturen');
      }
    } catch (e: any) {
      setError('Er is een fout opgetreden: ' + e.message);
    }
    ...
    <div className="page-container">
      ...
      <main style={{ position: 'relative', zIndex: 1, maxWidth: '800px', margin: '0 auto', padding: '2rem' }}>
        ...
        <div className="settings-section">
          <div className="settings-section-header">
            <span style={{ fontSize: '1.25rem' }}>📧</span>
            <h3 className="settings-section-title">E-mailadres</h3>
          </div>
          <div className="settings-section-body">
            ...
            {email && (
              <div style={{ marginTop: '1rem' }}>
                {user?.emailVerified ? (
                  <div style={{ padding: '0.75rem 1rem', background: 'rgba(52,211,153,0.1)', borderRadius: '12px', color: '#34d399', fontSize: '0.875rem' }}>
                    ✅ Je e-mailadres is geverifieerd
                  </div>
                ) : (
                  <div style={{ padding: '0.75rem 1rem', background: 'rgba(251,191,36,0.1)', borderRadius: '12px', color: '#fbbf24', fontSize: '0.875rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
                    <span>⏳ Je e-mailadres is nog niet geverifieerd</span>
                    <button
                    ...
                    >
                      {sendingVerification ? 'Versturen...' : 'Verstuur verificatie'}
                    </button>
                  </div>
                )}
              </div>
            ...
            <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', marginTop: '1rem' }}>
              💡 Je e-mailadres wordt gebruikt voor wachtwoord reset en belangrijke meldingen.
            </p>
          </div>
        </div>
